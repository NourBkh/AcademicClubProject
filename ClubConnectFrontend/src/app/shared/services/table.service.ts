...
export class TableService {
    ...
    deleteSingleData(name: string) {
        const tableItem = this.userData.slice(); // Make a copy of the original array

    const index = tableItem.findIndex(item => item.name === name);
    if (index !== -1) {
        tableItem.splice(index, 1); // Remove the item at the found index
    }
